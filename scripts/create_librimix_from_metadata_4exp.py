...
def create_librimix(librispeech_dir, wham_dir, out_dir, metadata_dir,
    ...
    #                    if 'info' not in file and 'dev' not in file and '360' not in file]
    # Create all parts of librimix

    # We only allow both min and max mode for test set
    csv_path = os.path.join(metadata_dir, f"libri{n_src}mix_test-clean.csv")
